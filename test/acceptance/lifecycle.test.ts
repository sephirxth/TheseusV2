...
const JUDGED: readonly (readonly [string, Verdict])[] = [
  ...
  // docs/requirements/bridge.md（第二稿，2026-08-19 重写为切换的体验）进仓后补判。
  // 逐句判过，9 句，没有一句是写给操作者的顺序要求：切走不办手续本身就是 negation（正是
  // U2-4 要的证据）；反面场景与案底的亲历叙述 = history；文档自述结构 = spec；
  // 排序讲道理 = design-method；画布与手机入口留到后面 = deferred。
  ['不填表、不登记、不挑分类，也不用先说一声', 'negation'],
  ['说正事之前先登记：这属于哪个任务', 'history'],
  ['每次切回来先翻几屏旧话', 'history'],
  ['于是大多数小念头我选择"记在脑子里"——**然后丢了**', 'history'],
  ['照痕迹那份的先例（"不丢不重：不配当用例、只能当前提"）', 'spec'],
  ['案底就是「经常记得回复、忘了留痕」', 'history'],
