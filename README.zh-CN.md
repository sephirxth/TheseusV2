...
| **桥** (bridge) | 活的连接：我在哪儿说话，话就到痕迹；每段对话开场有交代；切换意图真的生效 | U25–U29 | 📋 需求已定（含反面场景） |
...
- **桥（U25–U29）** —— 设计与实现：把系统接到真实对话发生的地方——经人的门捕捉、开场交代、跨多段并行会话真正生效的意图切换、断档重连不丢不重。
- **环（U20–U24）** —— 设计与实现：闭上「发生 → 看见 → 动手 → 留痕」，该动的自己动，撤不回的先问。
- **意图画布** —— 把整棵树摊在一张可拖拽的无限桌面上；第二入口，等桥立住后立需求。
